...
LAYOUT_SCRIPT = SCRIPT_DIR / "apply_projector_layout.py"
...
def apply_layout(tex_files: list[Path]) -> int:
    ...
    relative_paths = [str(path.resolve()) for path in tex_files]
...
def main() -> int:
    ...
    if args.paths:
        ...
        tex_files = sorted(OBJECTIVES_DIR.glob(DEFAULT_GLOB))
